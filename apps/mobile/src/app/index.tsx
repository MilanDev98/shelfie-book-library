...
export default function ScanHomeScreen() {
  ...
  const resetHome = () => { setScreen('home'); setHasPhoto(false); setPhotoUri(null); setPhotoAsset(null); setRequestError(null); setActiveBookId(null); setAnalysisTruncated(false); };
  ...
  const discardBook = (bookId: string) => {
    const discarded = books.find((book) => book.id === bookId);
    setScanBooks((existing) => existing?.filter((book) => book.id !== bookId) ?? null);
    if (discarded?.catalogId) {
      setConfirmedCatalogIds((ids) => ids.filter((catalogId) => catalogId !== discarded.catalogId));
    }
    setActiveBookId(null);
    setReviewSaveError(null);
    setReviewIndex(0);
